...
const garageDoorOutput1GateCapabilityID = 'garagedoor_closed.output1.gate'
const garageDoorOutput2FreeCapabilityID = 'garagedoor_closed.output2.free'
const garageDoorOutput1FreeCapabilityID = 'garagedoor_closed.output1.free'
const garageDoorOutput2GateCapabilityID = 'garagedoor_closed.output2.gate'
...
const getGarageDoorCapabilityId = app => {
  if (app.configuration.output1Only) return garageDoorCapabilityID
  if (app.configuration.output1Gate) return garageDoorOutput1GateCapabilityID
  if (app.configuration.output2Gate) return garageDoorOutput2GateCapabilityID
...
class RemootioDevice extends Device {
  ...
  async onInit () {
    ...
    this.onRemootioIncommingMessage = (frame, decryptedPayload) => {
      ...
      if (decryptedPayload.response !== undefined) { // it's a response frame to an action
        ...
        if (decryptedPayload.response.type === 'QUERY') {
          ...
          if (decryptedPayload.response.success) {
            ...
            this.setCapabilityValue(getGarageDoorCapabilityId(this), getState(logicFlipped, decryptedPayload.response.state !== 'open'))
          } else {
      ...
      if (decryptedPayload.event !== undefined) { // it's a event frame containing a log entry from Remootio
        ...
        if (decryptedPayload.event.type === 'StateChange') {
          ...
          this.setCapabilityValue(getGarageDoorCapabilityId(this), getState(logicFlipped, decryptedPayload.event.state !== 'open'))
        }
    ...
    if (this.configuration.output1Gate && this.configuration.output2Free) {
      // add garagedoor_closed.output1.gate ui toggle listener
      ...
      // add garagedoor_closed.output2.free ui toggle listener
    ...
    if (this.configuration.output1Free && this.configuration.output2Gate) {
      // add garagedoor_closed.output1.free ui toggle listener
      ...
      // add garagedoor_closed.output2.gate ui toggle listener
  ...
  async onSettings ({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('logicFlipped')) {
      ...
      const currentCapabilityValue = this.getCapabilityValue(getGarageDoorCapabilityId(this))
      ...
      this.setCapabilityValue(getGarageDoorCapabilityId(this), newCapabilityValue)
