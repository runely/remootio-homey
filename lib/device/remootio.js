 ...
 * @property {object} homey Homey instance
...
class RemootioDevice {
  ...
  constructor (options) {
    ...
    reconnectMaxCount = homey.getSetting('maxReconnectRetries') || 3

    logInfo = (...message) => homey.log(`[${homey.getName()}]${homey.getSetting('logicFlipped') ? ' [logic flipped]' : ''}`, `[${message.join('] [')}]`)
    logError = (error, ...message) => homey.error(`[${homey.getName()}]${homey.getSetting('logicFlipped') ? ' [logic flipped]' : ''}`, `[${error}]`, `[${message.join('] [')}]`)
  ...
  initialize (addListeners = true) {
    ...
    reconnectCount = 0
    ipaddress = homey.getSetting('ipaddress')
    secretKey = homey.getSetting('secretKey')
    authKey = homey.getSetting('authKey')
  ...
  onRemootioDisconnect (error) {
    if (!error) {
      if (reconnectCount >= reconnectMaxCount) {
        homey.setUnavailable('Remootio has disconnected. Check your WiFi connection to the device. Too many failed reconnect attempts...')
        ...
        homey.setUnavailable('Remootio has disconnected. Check your WiFi connection to the device')
        logInfo('onRemootioDisconnect', 'Disconnected from Remootio device')
      } */
    } else {
  ...
  onRemootioIncommingMessage (frame, decryptedPayload) {
    ...
    const logicFlipped = homey.getSetting('logicFlipped')
    const knownTypes = [
      'TRIGGER',
      'QUERY',
      'OPEN',
    ...
    if (decryptedPayload.response !== undefined) { // it's a response frame to an action
      ...
      if (decryptedPayload.response.type === 'QUERY') {
        ...
        if (decryptedPayload.response.success) {
          ...
          homey.setCapabilityValue('garagedoor_closed', getState(logicFlipped, decryptedPayload.response.state !== 'open'))
        } else {
    ...
    if (decryptedPayload.event !== undefined) { // it's a event frame containing a log entry from Remootio
      ...
      if (decryptedPayload.event.type === 'StateChange') {
        ...
        homey.setCapabilityValue('garagedoor_closed', getState(logicFlipped, decryptedPayload.event.state !== 'open'))
      }
  ...
  changeState (state) {
    ...
    */
    if (homey.getSetting('logicFlipped')) {
      if (!state) {
        remootio.sendClose()
