...
const getOutputsReadable = (configuration, capabilities) => {
  if (configuration.output1Only) return 'Output 1: gate impulse control'
  if (configuration.output1Gate && configuration.output2Free) return 'Output 1: gate impulse control, Output 2: free relay output'
  if (configuration.output1Free && configuration.output2Gate) return 'Output 1: free relay output, Output 2: gate impulse control'
...
class RemootioDevice extends Device {
  ...
  async onInit () {
    ...
    this.configuration = {
      output1Only: capabilities.includes(garageDoorCapabilityID),
      output1Gate: capabilities.includes(garageDoorOutput1GateCapabilityID),
      output1Free: capabilities.includes(garageDoorOutput1FreeCapabilityID),
      output2Gate: capabilities.includes(garageDoorOutput2GateCapabilityID),
      output2Free: capabilities.includes(garageDoorOutput2FreeCapabilityID)
    }
    this.configuration.outputsReadable = getOutputsReadable(this.configuration, capabilities)

    this.log(`${this.getName()} -- onInit -- Device has been initialized with configuration: ${this.configuration.outputsReadable}`)
    ...
    this.initialize = (addListeners = true) => {
      ...
      this.log(`${this.getName()} -- Connecting with IP:${ipaddress} SecretKey:'${secretKey}' AuthKey:'${authKey}'`)
    ...
    this.onRemootioIncommingMessage = (frame, decryptedPayload) => {
      ...
      if (decryptedPayload.response !== undefined) { // it's a response frame to an action
        ...
        if (decryptedPayload.response.type === 'QUERY') {
          ...
          if (decryptedPayload.response.success) {
            ...
            this.setCapabilityValue(this.getGarageDoorCapabilityId(), getState(logicFlipped, decryptedPayload.response.state !== 'open'))
          } else {
      ...
      if (decryptedPayload.event !== undefined) { // it's a event frame containing a log entry from Remootio
        ...
        if (decryptedPayload.event.type === 'StateChange') {
          ...
          this.setCapabilityValue(this.getGarageDoorCapabilityId(), state)
    ...
    this.getGarageDoorState = () => {
      const logicFlipped = this.getSetting('logicFlipped')
      const capabilityValue = this.getCapabilityValue(this.getGarageDoorCapabilityId())
      return getState(logicFlipped, capabilityValue)
    }

    this.getFreeRelayState = () => {
      const capabilityValue = this.getCapabilityValue(this.getFreeRelayCapabilityId())
      return capabilityValue
    }

    this.getGarageDoorCapabilityId = () => {
      if (this.configuration.output1Only) return garageDoorCapabilityID
      if (this.configuration.output1Gate) return garageDoorOutput1GateCapabilityID
      if (this.configuration.output2Gate) return garageDoorOutput2GateCapabilityID
      return garageDoorCapabilityID
    }

    this.getFreeRelayCapabilityId = () => {
      if (this.configuration.output1Free) return garageDoorOutput1FreeCapabilityID
      if (this.configuration.output2Free) return garageDoorOutput2FreeCapabilityID
      return null
    }
    // #endregion
    ...
    if (this.configuration.output1Gate && this.configuration.output2Free) {
      // add garagedoor_closed.output1_gate ui toggle listener
      this.registerCapabilityListener(garageDoorOutput1GateCapabilityID, async value => {
        this.log(`${this.getName()} -- ${garageDoorOutput1GateCapabilityID} capability listener -- Triggered with value:`, value)
        this.changeState(value)
      })

      // add garagedoor_closed.output2_free ui toggle listener
      this.registerCapabilityListener(garageDoorOutput2FreeCapabilityID, async value => {
        this.log(`${this.getName()} -- ${garageDoorOutput2FreeCapabilityID} capability listener -- Triggered with value:`, value)
        this.remootio.sendTriggerSecondary()
      })
    }

    if (this.configuration.output1Free && this.configuration.output2Gate) {
      // add garagedoor_closed.output1_free ui toggle listener
      this.registerCapabilityListener(garageDoorOutput1FreeCapabilityID, async value => {
        this.log(`${this.getName()} -- ${garageDoorOutput1FreeCapabilityID} capability listener -- Triggered with value:`, value)
        this.remootio.sendTriggerSecondary()
      })

      // add garagedoor_closed.output2_gate ui toggle listener
      this.registerCapabilityListener(garageDoorOutput2GateCapabilityID, async value => {
        this.log(`${this.getName()} -- ${garageDoorOutput2GateCapabilityID} capability listener -- Triggered with value:`, value)
        this.changeState(value)
      })
    }
  }
  ...
  async onSettings ({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('logicFlipped')) {
      ...
      const currentCapabilityValue = this.getCapabilityValue(this.getGarageDoorCapabilityId())
      ...
      this.setCapabilityValue(this.getGarageDoorCapabilityId(), newCapabilityValue)
