  ...
  async onInit () {
    ...
    this.onRemootioDisconnect = error => {
      if (error) {
        ...
        this.warn(`${this.getName()} -- onRemootioDisconnect -- Device has disconnected`)
      }
      ...
      if (this.reconnectCount >= maxReconnectRetries) {
        ...
        this.setUnavailable(error || this.currentError ? `${error || this.currentError} -- ${this.homey.__('device.disconnected.maxRetries')}` : this.homey.__('device.disconnected.maxRetries'))
        this.error(`${this.getName()} -- onRemootioDisconnect -- Disconnected from device --${error || this.currentError ? ` ${error || this.currentError} --` : ''} Too many failed reconnect attempts: ${this.reconnectCount} / ${maxReconnectRetries}`)
      } else {
