  ...
  async onInit () {
    ...
    this.onRemootioDisconnect = error => {
      if (error) {
        ...
        this.log(`${this.getName()} -- onRemootioDisconnect -- Device has disconnected due to normal circumstances`)
      }
      ...
      if (this.reconnectCount >= maxReconnectRetries) {
        ...
        this.setUnavailable(error || this.currentError ? `${error || this.currentError} -- ${this.homey.__('device.disconnected.maxRetries')}` : this.homey.__('device.disconnectedmaxRetries'))
        this.error(`${this.getName()} -- onRemootioDisconnect -- Disconnected from device -- ${error || this.currentError ? `${error || this.currentError} --` : ''} Too many failed reconnect attempts: ${this.reconnectCount} out of ${maxReconnectRetries}`)
        this.remootio.disconnect()
      } else {
