...
class RemootioDevice extends Device {
  ...
  async onInit () {
    ...
    this.removeAllListeners = () => {
      this.remootio.removeAllListeners('connecting')
      this.remootio.removeAllListeners('connected')
      this.remootio.removeAllListeners('authenticated')
      this.remootio.removeAllListeners('error')
      this.remootio.removeAllListeners('disconnect')
      this.remootio.removeAllListeners('incomingmessage')
      this.log(`${this.getName()} -- removeAllListeners -- All listeners removed`)
    }
    // #endregion
  ...
  async onSettings ({ oldSettings, newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('ipaddress') || changedKeys.includes('secretKey') || changedKeys.includes('authKey')) {
      // since the new settings isn't saved until this function is finished, reconnect after 100ms
      setTimeout(() => {
        this.removeDevice()
        setTimeout(() => {
