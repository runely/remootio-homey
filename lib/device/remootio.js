  ...
  async onInit () {
    ...
    this.onRemootioDisconnect = error => {
      ...
      if (this.reconnectCount >= maxReconnectRetries) {
        ...
        this.warn(`${this.getName()} -- onRemootioDisconnect -- Disconnected from device --${error || this.currentError ? ` ${error || this.currentError} --` : ''} Too many failed reconnect attempts in a short time: ${this.reconnectCount} / ${maxReconnectRetries}. Next reconnect attempt will be ${next}`)
