...
class RemootioDeviceAPI extends Device {
  ...
  async onInit () {
    this.registerCapabilityListener(garageDoorCapabilityId, async (value) => {
      ...
      if (!response.success) {
        ...
        throw new Error(`Toggle failed with warning: ${response.data.errorMessage}`)
      }
  ...
  /**
   * queryDevice is called on device init, after a successful toggle and every x hours (if enabled)
   */
  async queryDevice () {
    const response = await this.queryStatus()
    if (!response.success) {
    ...
    // set device as available since we have contact with the device
